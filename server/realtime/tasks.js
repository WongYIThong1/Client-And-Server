...
import { authenticatedConnections, clientSystemInfo, runningTasks, taskAssignments, addTaskToConnection, removeTaskFromConnection, findConnectionByMachine } from '../stores.js';
...
/**
 * Machine信息缓存
 * Map<machineId, {machine, timestamp}>
 */
const machineCache = new Map();
const CACHE_TTL = 5 * 60 * 1000; // 5分钟缓存

// 启动machine缓存定期清理
let machineCacheCleanupInterval = null;
function startMachineCacheCleanup() {
  if (machineCacheCleanupInterval) {
    return; // 已经启动
  }
  
  machineCacheCleanupInterval = setInterval(() => {
    const now = Date.now();
    let cleanedCount = 0;
    for (const [machineId, cached] of machineCache.entries()) {
    ...
    if (cleanedCount > 0) {
      console.log(`[cache] Cleaned up ${cleanedCount} expired machine cache entries`);
    }
  }, 5 * 60 * 1000); // 每5分钟清理一次
}

// 停止machine缓存清理
export function stopMachineCacheCleanup() {
  if (machineCacheCleanupInterval) {
    clearInterval(machineCacheCleanupInterval);
    machineCacheCleanupInterval = null;
  }
}
 ...
 * @returns {Promise<{id: string, name: string | null, hwid: string | null} | null>}
 */
async function getMachineInfo(machineId, userId) {
...
function findTargetWebSocketForMachine(userId, machine) {
  ...
  // 使用索引进行O(1)查找
  return findConnectionByMachine(userId, machine);
}
...
/**
 * 任务创建事件队列（缓冲处理，避免同时处理大量事件）
 */
const taskCreationQueue = [];
let isProcessingQueue = false;
const QUEUE_PROCESS_INTERVAL = 500; // 每500ms处理一次队列
const MAX_BATCH_SIZE = 20; // 每批最多处理20个任务

/**
 * 处理任务创建队列
 */
async function processTaskCreationQueue() {
  if (isProcessingQueue || taskCreationQueue.length === 0) {
    return;
  }
  ...
  try {
    // 获取一批任务进行处理
    const batch = taskCreationQueue.splice(0, MAX_BATCH_SIZE);

    // 并行处理这批任务
    const promises = batch.map(async (taskData) => {
      try {
        await dispatchTaskToMachine(taskData);
      } catch (error) {
        console.error(`[realtime:tasks] Error processing queued task ${taskData.taskId}:`, error);
      }
    });

    await Promise.allSettled(promises);

    if (taskCreationQueue.length > 0) {
      console.log(`[realtime:tasks] Processed ${batch.length} tasks, ${taskCreationQueue.length} remaining in queue`);
    }
  } catch (error) {
    console.error('[realtime:tasks] Error processing task creation queue:', error);
  } finally {
 ...
 */
async function dispatchTaskToMachine(taskData) {
  const {
    taskId,
    machineId,
    taskName,
  ...
  // 使用缓存的machine信息查询
  const machine = await getMachineInfo(machineId, userId);

  if (!machine) {
    console.error(`[realtime:tasks] Failed to find machine ${machineId} for user ${userId}`);
    return;
  }

  // 查找对应的 WebSocket 连接（使用索引，O(1)复杂度）
      ...
      if (!stateError && machineState && machineState.status === 'Active') {
        ...
        setTimeout(async () => {
          try {
            ...
            const retryMachine = await getMachineInfo(machineId, userId);
            if (!retryMachine) return;

            const retryWs = findTargetWebSocketForMachine(userId, retryMachine);
...
export function startTaskRealtimeListener() {
  ...
  // 启动任务创建队列处理器
  const queueProcessorInterval = setInterval(processTaskCreationQueue, QUEUE_PROCESS_INTERVAL);
  ...
  const channel = supabase
    ...
    .on(
      ...
      async (payload) => {
        ...
        console.log(`[realtime:tasks] Task assigned: ${taskId} to machine ${machineId} (user ${userId}) - queued for processing`);

        // 将任务加入队列，批量处理（避免同时处理大量事件）
        taskCreationQueue.push({
          taskId,
          machineId,
          taskName,
          listFile,
    ...
    .on(
      ...
      async (payload) => {
        ...
        try {
          ...
          // 优化：添加到连接的索引中
          addTaskToConnection(targetWs, taskId);
        } catch (error) {
    ...
    .subscribe((status, err) => {
      if (err) {
        console.error('[realtime:tasks] Channel error:', err);
      }
      
      if (status === 'SUBSCRIBED') {
        console.log('[realtime:tasks] Channel subscribed successfully');
      } else if (status === 'CHANNEL_ERROR') {
        console.error('[realtime:tasks] Channel error occurred, will attempt to reconnect');
        // 延迟重连，避免频繁重试
        setTimeout(() => {
          try {
            channel.unsubscribe();
            ...
            console.error('[realtime:tasks] Failed to reconnect:', reconnectError);
          }
        }, 5000); // 5秒后重连
      } else if (status === 'TIMED_OUT') {
        console.warn('[realtime:tasks] Channel timed out, will attempt to reconnect');
        setTimeout(() => {
          try {
            channel.unsubscribe();
            channel.subscribe();
          } catch (reconnectError) {
            console.error('[realtime:tasks] Failed to reconnect after timeout:', reconnectError);
          }
        }, 5000);
      } else {
  ...
  // 返回 channel 和 interval，以便清理
  return { channel, queueProcessorInterval };
}
