import { authenticatedConnections, cleanupConnection, clientSystemInfo, clientIPs, runningTasks, getTasksByConnection, removeTaskFromConnection } from '../stores.js';
...
export function setupConnection(ws) {
  ...
  const NEXT_PROGRESS_DELAY_MS = 300000; // 之后每次请求进度的间隔（优化：从120秒改为300秒/5分钟，大幅减少数据库请求）
  let heartbeatCount = 0; // 心跳计数器，用于控制machine检查频率
  const MACHINE_CHECK_INTERVAL = 2; // 每2次心跳（60秒）检查一次machine
  
  // 优化：进度请求定时器 - 使用连接索引，避免遍历所有任务
  // 每30秒向客户端请求运行中任务的进度
  const progressRequestInterval = setInterval(() => {
    ...
    // 优化：直接获取该连接的任务列表，无需遍历所有任务
    const taskIds = getTasksByConnection(ws);
    if (taskIds.size === 0) {
      return; // 该连接没有运行中的任务
    }
    ...
    for (const taskId of taskIds) {
      ...
      if (!taskInfo || taskInfo.ws !== ws || taskInfo.userId !== connInfo.userId) {
        // 任务不存在或不属于此连接，从索引中移除
        removeTaskFromConnection(ws, taskId);
        continue;
      }
      ...
      if (!taskInfo.lastProgressRequest || (now - taskInfo.lastProgressRequest) >= requiredDelay) {
        try {
          ...
          // 只在第一次请求或每10次请求时输出日志，减少日志噪音
          if (requestCount === 0 || (requestCount + 1) % 10 === 0) {
