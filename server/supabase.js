...
import { cleanupCompletedTasks } from './stores.js';
...
// 任务验证缓存（减少数据库查询）
// Map<taskId, {userId, timestamp}>
const taskValidationCache = new Map();
const TASK_CACHE_TTL = 60000; // 缓存60秒

// 启动任务验证缓存定期清理
let taskCacheCleanupInterval = null;
export function startTaskCacheCleanup() {
  if (taskCacheCleanupInterval) {
    return; // 已经启动
  }
  
  taskCacheCleanupInterval = setInterval(() => {
    const now = Date.now();
    let cleanedCount = 0;
    for (const [taskId, cached] of taskValidationCache.entries()) {
      if (now - cached.timestamp > TASK_CACHE_TTL) {
        taskValidationCache.delete(taskId);
        cleanedCount++;
      }
    }
    if (cleanedCount > 0) {
      console.log(`[cache] Cleaned up ${cleanedCount} expired task validation cache entries`);
    }
  }, 5 * 60 * 1000); // 每5分钟清理一次
}

// 停止任务验证缓存清理
export function stopTaskCacheCleanup() {
  if (taskCacheCleanupInterval) {
    clearInterval(taskCacheCleanupInterval);
    taskCacheCleanupInterval = null;
  }
}
 ...
 * @param {string} extraSalt - 额外的盐值（可选，用于确保唯一性）
 * @returns {string} HWID
 */
function generateHWID(userId, ip, machineName, extraSalt = '') {
  const combined = `${userId}||${ip || ''}||${machineName || ''}||${extraSalt}`;
...
export async function saveOrUpdateMachine(userId, apiKey, machineInfo) {
  try {
    ...
    if (existingMachine) {
      // 检查 hwid 是否已经被其他机器使用（避免唯一约束冲突）
      let hwidToUse = finalHWID;
      if (finalHWID && existingMachine.hwid !== finalHWID) {
        // 需要更新 hwid，先检查是否冲突
        const { data: conflictingMachine, error: conflictCheckError } = await supabase
          .from('machines')
          .select('id, user_id')
          .eq('hwid', finalHWID)
          .neq('id', existingMachine.id) // 排除当前机器
          .maybeSingle();

        if (conflictCheckError && conflictCheckError.code !== 'PGRST116') {
          console.error('Error checking hwid conflict:', conflictCheckError);
        }

        if (conflictingMachine) {
          // hwid 已被其他机器使用
          if (conflictingMachine.user_id === userId) {
            // 同一用户的另一台机器，可能是重复记录
            // 保留原有 hwid，避免冲突
            console.warn(`HWID ${finalHWID} already used by another machine (${conflictingMachine.id}) of the same user. Keeping existing HWID.`);
            hwidToUse = existingMachine.hwid || finalHWID;
          } else {
            // 不同用户的机器，保留原有 hwid
            console.warn(`HWID ${finalHWID} already used by another user's machine. Keeping existing HWID.`);
            hwidToUse = existingMachine.hwid || finalHWID;
          }
        }
        // 如果没有冲突，使用新的 hwid
      } else if (!finalHWID && existingMachine.hwid) {
        // 客户端没有提供 hwid，但数据库中有，保留原有的
        hwidToUse = existingMachine.hwid;
      }
      ...
      if (updateError) {
        // 如果仍然有唯一约束错误，尝试不更新 hwid
        if (updateError.code === '23505' && updateError.message?.includes('hwid')) {
          console.warn(`HWID conflict detected, retrying without hwid update`);
          const machineDataWithoutHWID = {
            ...machineData,
            hwid: existingMachine.hwid // 保留原有 hwid
          };
          const { error: retryError } = await supabase
            .from('machines')
            .update(machineDataWithoutHWID)
            .eq('id', existingMachine.id);
      ...
      // 创建新记录前，检查 hwid 是否已被使用
      let hwidToUse = finalHWID;
      if (finalHWID) {
        const { data: existingHWIDMachine, error: hwidCheckError } = await supabase
          .from('machines')
          .select('id, user_id')
          .eq('hwid', finalHWID)
          .maybeSingle();

        if (hwidCheckError && hwidCheckError.code !== 'PGRST116') {
          console.error('Error checking hwid before insert:', hwidCheckError);
        }

        if (existingHWIDMachine) {
          // hwid 已被使用
          if (existingHWIDMachine.user_id === userId) {
            // 同一用户的机器，可能是重复记录，生成新的 hwid
            console.warn(`HWID ${finalHWID} already exists for user ${userId}, generating new HWID`);
            hwidToUse = generateHWID(userId, ip, machineName, Date.now().toString()); // 添加时间戳确保唯一
          } else {
            // 不同用户的机器，生成新的 hwid
            console.warn(`HWID ${finalHWID} already exists for another user, generating new HWID`);
            hwidToUse = generateHWID(userId, ip, machineName, Date.now().toString());
          }
        }
      }
      ...
      if (insertError) {
        // 如果仍然有唯一约束错误，尝试不设置 hwid
        if (insertError.code === '23505' && insertError.message?.includes('hwid')) {
          console.warn(`HWID conflict on insert, retrying without hwid`);
          const machineDataWithoutHWID = {
            ...machineData,
            hwid: null // 不设置 hwid
          };
          ...
          return { success: true, machineId: retryInserted?.id || null, created: true, updated: false };
        }
...
/**
 * 写入或更新 task_url 表中的记录（优化：使用批量upsert）
 * @param {string} userId - 用户ID
...
export async function upsertTaskUrlResults(userId, taskId, urlResults) {
  try {
    ...
    // 优化：检查任务验证缓存，减少数据库查询
    const cached = taskValidationCache.get(taskId);
    if (cached && (Date.now() - cached.timestamp) < TASK_CACHE_TTL) {
      ...
      // 更新缓存
      taskValidationCache.set(taskId, {
        userId,
        timestamp: Date.now()
      });
    }
    ...
    // 优化：过滤掉离线网站，准备批量数据
    const recordsToUpsert = urlResults
      .filter(result => result.status !== 'offline') // 跳过离线网站
      .map(result => ({
        task_id: taskId,
        domains: result.domain || null,
        waf: result.waf || null,
    ...
    if (recordsToUpsert.length === 0) {
      return { success: true }; // 没有需要写入的记录
    }

    // 优化：使用批量处理（先查询现有记录，然后批量插入和更新）
    // 获取所有需要处理的域名列表
    const domains = recordsToUpsert.map(r => r.domains).filter(d => d !== null);
    
    if (domains.length === 0) {
      return { success: true };
    }

    // 修复：分批查询现有记录，避免 HeadersOverflowError
    // 每次最多查询 100 个域名（避免请求头过大）
    const BATCH_QUERY_SIZE = 100;
    const domainToIdMap = new Map();
    let hasQueryError = false;

    // 分批查询现有记录（无论域名数量多少，都使用分批查询）
    // 即使有1000+个域名，也可以通过分批查询高效处理
    const totalBatches = Math.ceil(domains.length / BATCH_QUERY_SIZE);
    if (domains.length > 200) {
      console.log(`[task_url] Processing ${domains.length} domains in ${totalBatches} batches (${BATCH_QUERY_SIZE} per batch)`);
    }

    for (let i = 0; i < domains.length; i += BATCH_QUERY_SIZE) {
      const batchDomains = domains.slice(i, i + BATCH_QUERY_SIZE);
      const batchNumber = Math.floor(i / BATCH_QUERY_SIZE) + 1;
      
      try {
        const { data: existingRecords, error: batchError } = await supabase
          .from('task_url')
          .select('id, domains')
          .eq('task_id', taskId)
          .in('domains', batchDomains);

        if (batchError && batchError.code !== 'PGRST116') {
          // 如果批次查询失败（可能是 HeadersOverflow），记录错误并跳过批量查询
          if (batchError.message?.includes('HeadersOverflow') || batchError.message?.includes('fetch failed') || batchError.details?.includes('HeadersOverflow')) {
            console.warn(`[task_url] Batch query failed due to header overflow (batch ${batchNumber}/${totalBatches}), falling back to individual upsert`);
            hasQueryError = true;
            break; // 停止批量查询，直接回退
          }
          console.error(`[task_url] Error querying batch ${batchNumber}/${totalBatches}:`, batchError);
          hasQueryError = true;
          continue; // 继续下一批
        }
        ...
        if (existingRecords) {
          existingRecords.forEach(record => {
            if (record.domains) {
              domainToIdMap.set(record.domains, record.id);
            }
          });
        }
        
        // 每处理10批或最后一批时输出进度（避免日志过多）
        if (batchNumber % 10 === 0 || batchNumber === totalBatches) {
          console.log(`[task_url] Query progress: ${batchNumber}/${totalBatches} batches completed (${Math.min(i + BATCH_QUERY_SIZE, domains.length)}/${domains.length} domains)`);
        }
      } catch (error) {
        console.error(`[task_url] Error in batch query ${batchNumber}/${totalBatches}:`, error);
        // 如果是 HeadersOverflow 错误，直接回退
        if (error.message?.includes('HeadersOverflow') || error.message?.includes('fetch failed') || error.details?.includes('HeadersOverflow')) {
          hasQueryError = true;
          break;
        }
      }
    ...
    // 如果查询失败（特别是 HeadersOverflow），回退到逐个处理
    if (hasQueryError) {
      console.warn(`[task_url] Batch query failed, falling back to individual upsert`);
      return await fallbackToIndividualUpsert(taskId, recordsToUpsert);
    }
    ...
    for (const record of recordsToUpsert) {
      if (!record.domains) continue;
      
      const existingId = domainToIdMap.get(record.domains);
      if (existingId) {
        // 需要更新
    ...
    // 批量插入新记录（也要分批，避免单次插入过多导致 HeadersOverflow）
    if (recordsToInsert.length > 0) {
      const INSERT_BATCH_SIZE = 200; // 每次最多插入 200 条
      ...
      for (let i = 0; i < recordsToInsert.length; i += INSERT_BATCH_SIZE) {
        ...
        if (insertError) {
          if (insertError.code === '23503') {
            // 任务已删除，静默返回成功
            console.log(`[task_url] Task ${taskId} no longer exists, skip writing url results`);
            return { success: true };
          }
          
          // 如果是 HeadersOverflow 错误，回退到逐个处理
          if (insertError.message?.includes('HeadersOverflow') || insertError.message?.includes('fetch failed') || insertError.details?.includes('HeadersOverflow')) {
            console.warn(`[task_url] Batch insert failed due to header overflow (batch ${Math.floor(i / INSERT_BATCH_SIZE) + 1}), falling back to individual upsert`);
            return await fallbackToIndividualUpsert(taskId, recordsToUpsert);
          }
          
          console.error(`[task_url] Batch insert error (batch ${Math.floor(i / INSERT_BATCH_SIZE) + 1}):`, insertError);
          // 如果批量插入失败，回退到逐个处理
          return await fallbackToIndividualUpsert(taskId, recordsToUpsert);
        }
      }
      ...
      for (let i = 0; i < recordsToUpdate.length; i += BATCH_SIZE) {
        ...
        for (const record of batch) {
          ...
          if (updateError) {
            ...
            console.error(`[task_url] Failed to update record for domain ${record.domains}:`, updateError);
          }
        }
      }
    }
...
/**
 * 回退方案：逐个处理 upsert（当批量操作失败时使用）
 * @param {string} taskId - 任务ID
 * @param {Array<object>} recordsToUpsert - 需要处理的记录数组
 * @returns {Promise<{success: boolean, error?: string}>}
 */
async function fallbackToIndividualUpsert(taskId, recordsToUpsert) {
  console.warn(`[task_url] Falling back to individual upsert for task ${taskId}`);
  ...
  for (const record of recordsToUpsert) {
    ...
    try {
      ...
      const { data: existing, error: queryError } = await supabase
        ...
        .select('id')
        .eq('task_id', taskId)
        .eq('domains', record.domains)
        .maybeSingle();

      if (queryError && queryError.code !== 'PGRST116') {
        console.error(`[task_url] Error querying for domain ${record.domains}:`, queryError);
        continue;
      }
      ...
      if (existing) {
        ...
        if (insertError) {
          ...
          console.error(`[task_url] Failed to insert record for domain ${record.domains}:`, insertError);
        }
      }
    } catch (error) {
      console.error(`[task_url] Error processing record for domain ${record.domains}:`, error);
    }
  }
 ...
 */
export function startTaskCleanupScheduler(intervalMs = 5 * 60 * 1000) {
  console.log(`[cleanup] Task cleanup scheduler started (interval: ${intervalMs / 1000}s)`);
  ...
  const cleanupInterval = setInterval(async () => {
    try {
      ...
      const { data: completedTasks, error } = await supabase
        ...
        .limit(1000); // 每次最多清理1000个任务

      if (error) {
        console.error('[cleanup] Error querying completed tasks:', error);
        return;
      }
      ...
      // 从内存中清理这些任务
      cleanupCompletedTasks(completedTaskIds);

      console.log(`[cleanup] Cleaned up ${completedTaskIds.length} completed/failed tasks from memory`);
    } catch (error) {
      console.error('[cleanup] Error in task cleanup scheduler:', error);
    }
  }, intervalMs);
